...
## 첫 실행 때 뜨는 `Can't Reach Server` (2026-08-20 원인 확정)

딥링크를 받은 Plex 가 **아직 미디어 서버에 연결되기 전**에 재생 대기열을 요청해서 난다.
두 번째부터는 안 난다. iOS 실기기에서는 `Item not known when attempting to build
decision` · `Failed to fetch play queue response` 문구로도 나온다 — **같은 뿌리다.**

**원인은 Plex 클라이언트 하나다. 서버 쪽이 아니다.** 다음으로 갈렸다:

> 서버에서 다른 기기가 **시청 중**이고(= 서버·프록시가 확실히 깨어 있음) 직전에
> `/identity` 요청까지 성공한 상태에서, **Plex 앱만 껐다가** 딥링크를 눌렀더니
> 같은 오류가 재현됐다.

그러므로 아래 셋은 전부 무관하다. **다시 시도하지 말 것.**

- **서버 예열** — 앱 부팅 때 Plex 서버를 미리 깨워도 소용없다(위 실험이 반증)
- **Custom server access URLs** — 이미 `https://plex.nugabox.com,https://nuplex.nugabox.com`
  로 설정돼 있다. 넣어도 증상은 그대로였다. 참고로 plex.tv 는 이 호스트에 서버의
  공개 포트를 붙여 `:13394` 로 광고한다. `:443` 도 정상 동작하지만 광고 목록엔 없다
- **`plex://preplay/`** — 2026-08-18 재확인. 위 절 참고

**셸에서 고칠 수 없다.** 링크를 넘기는 순간 OS 가 우리 앱을 백그라운드로 내리므로
재시도할 수 없고, **다른 앱을 백그라운드로 띄우는 API 는 두 플랫폼 모두 없다**
(iOS 는 앱 전환뿐, Android 10+ 는 백그라운드 액티비티 시작을 막는다).

확실한 회피는 하나뿐이다 — **Plex 를 한 번 띄워 두는 것.**
