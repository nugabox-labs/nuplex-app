...
## 6-1. 수정 후 재검증 (2026-09-01 19:00, 같은 실기기)

새 APK 를 깔고(데이터 유지) 운영 배포가 끝난 뒤 같은 순서로 다시 돌렸다.

| 확인 | 결과 |
| --- | --- |
| 콜드 스타트 (토큰 그대로) | `푸시 토큰 등록 완료` ✅ — 전에는 조용히 건너뛰던 자리다 |
| "나가기" | `푸시 토큰 해제 (200)` ✅ — 세션이 살아 있을 때 나갔다 |
| 로그아웃 직후 `notifyWebReady` | `아직 로그인 전이라 토큰 등록을 미룹니다 (307)` ✅ — 매번 등록으로 바꿔도 로그인 전에는 조용히 유예된다 |
| 백송이로 로그인 | `푸시 토큰 등록 완료` ✅ |
| V1 공지 대상 백송이(14) | 백송이 기기에 **도착** ✅ (전에는 안 왔다) |
| V2 공지 대상 NUGA(1) | 백송이 기기에 **안 옴** ✅ (전에는 왔다) |

발송 응답의 `queued` 도 따라 움직였다 — NUGA 2 → **1**, 백송이 4 → **5**.
이 기기가 서버에서 실제로 프로필을 갈아탔다는 뜻이다.

관리자 화면도 같은 기기에서 봤다.

- 탭 줄이 상태 표시줄에서 떨어졌다 ✅ (`.page-safe` 가 배포되면서)
- 제목 입력칸에 포커스를 줘도 화면 배율이 그대로다 ✅ — 탭 줄 위치·글자 크기·요소
  폭이 포커스 전후로 같고 키보드만 올라온다

**미검증**: 확대 문제도 상단 잘림도 원래 iOS 에서 난 것인데, 위 확인은 Android 다.
Android WebView 는 원래 포커스로 확대하지 않으므로 **이 확인이 iOS 를 대신하지 못한다.**
빌드 산출물에 `user-scalable=no` 와 `font-size:16px` 이 들어간 것,
`.page-safe` 가 `2.5rem + env(safe-area-inset-top)` 인 것까지만 확인했다.
iOS 실기기·시뮬레이터 확인은 [Desktop] 몫으로 남긴다.
