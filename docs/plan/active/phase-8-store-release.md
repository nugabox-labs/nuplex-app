...
- [x] [Code] **등록정보 애셋과 텍스트를 API 로 반영했다** (2026-09-01)
      - 아이콘 512×512 · 그래픽 이미지 1024×500 · 휴대전화 스크린샷 4장
      - 확인: `inspect` 재실행 결과 `icon: 1장` · `featureGraphic: 1장` ·
        `phoneScreenshots: 4장` · `자세한 설명 709자`
      - 방법은 `.github/workflows/play-listing.yml` — 콘솔의 애셋 업로드는 OS 파일
        선택창을 띄워 자동화가 닿지 않고, 콘솔이 Trusted Types 로 페이지 안 JS 실행도
        막는다. 자격증명은 `PLAY_SERVICE_ACCOUNT_JSON` 하나로 족했다
      - **텍스트도 저장소로 옮겼다**(`resources/store/play/listing-ko.txt`).
        새 edit 은 콘솔 임시보관함이 아니라 **게시된** 값에서 시작하는데 그때 설명이
        비어 있었다 — 이미지만 올리고 커밋했다면 빈 설명이 게시될 뻔했다
      - 스크린샷에서 **뺀 것 둘** — `/profile` 은 실제 이용자들의 이름과 프로필 사진이
        그대로 나온다. `/guide` 는 "VPN 설치 → 지역을 미국으로" 절차가 화면 한가운데에
        있다. 둘 다 스토어에 올릴 것이 아니다
...
- [x] [Code] **스토어 등록정보 스크린샷** — 4장 반영 완료 (2026-09-01)
      - 실기기 없이 해결했다. 로그인이 필요 없는 화면 둘(`/welcome` · `/privacy`)과
        셸 자체 화면 둘(알림 권한 · 오프라인)을 헤드리스 Chrome 으로 찍었다.
        `--force-device-scale-factor=2` 라 540×960 뷰포트가 1080×1920 으로 나온다
      - 넷 다 1,080px 이상이라 프로모션 조건(4장 이상)도 채운다
      - 만드는 법은 `resources/store/play/README.md`
