...
## Android — 웹 주소를 바꾼 뒤 첫 실행에만 브릿지가 안 붙는다

`MainActivity.originRules()` 는 **액티비티가 만들어지는 시점의** `webBaseUrl` 로 주입
허용 origin 을 정한다. 그런데 그 값은 셸 JS 가 부팅하면서 갱신한다. 순서가 이렇다.

```
액티비티 생성 → 옛 주소로 origin 규칙 확정 → 셸 JS 부팅 → 새 주소를 Preferences 에 기록
→ 웹뷰가 새 주소로 이동 → 그 origin 은 규칙에 없다 → 주입 안 됨
```

**다음 실행부터는 정상이다.** 새 주소가 이미 저장돼 있기 때문이다. 개발 중 주소를
바꿨을 때 한 번 겪고 지나가므로, 브릿지가 안 붙으면 **앱을 한 번 껐다 켜 볼 것.**

운영에서 도메인을 옮길 때도 같은 일이 벌어진다. 이동 직후 첫 실행에서 `openInPlex` ·
`notifyWebReady` 가 동작하지 않는다. 도메인 이전은 앱 배포와 함께 하는 편이 안전하다.
